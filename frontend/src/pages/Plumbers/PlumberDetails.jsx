import { useState } from "react";
import defaultImg from "../../assets/images/default0.png";
import starIcon from "../../assets/images/Star.png";
import PlumberAbout from "./PlumberAbout";
import Feedback from "./Feedback";
import SidePanel from "./SidePanel";
import { useParams } from "react-router-dom";
import { useBackendGet, useFetch } from "../../lib/hooks";
import _ from "lodash";

export default function PlumberDetails() {
  const [tab, setTab] = useState("about");

  const { id } = useParams();
  const [plumber, fetchData] = useBackendGet(`plumbers/${id}`, {});

  return (
    <section>
      <div className="max-w-[1170px] px-5 mx-auto">
        <div className="grid md:grid-cols-3 gap-[50px]">
          <div className="md:col-span-2">
            <div className="flex items-center gap-5">
              <figure className="max-w-[200px] max-h-[200px]">
                <img src={plumber.photo ? plumber.photo : defaultImg} alt="" />
              </figure>
              <div>
                <span
                  className="bg-[#ccf0f3] text-irisBlueColor py-1 px-6 lg:py-2 lg:px-6 text-[12px] leading-4 lg:text-[16px]
                lg:leading-7 font-semibold rounded"
                >
                  {plumber.role}
                </span>
                <h3 className="text-HeadingColor text-[22px] leading-9 mt-3 font-bold">
                  {_.capitalize(plumber.name)}
                </h3>

                <div className="flex items-center gap-[6px]">
                  <span className="flex items-center gap-[6px] text-[14px] leading-5 lg:text-[16px] lg:leading-7 font-semibold text-headingColor">
                    <img src={starIcon} alt="" />
                    {plumber.averageRating}
                  </span>
                  <span className=" text-[14px] leading-5 lg:text-[16px] lg:leading-7 font-[400] text-textColor">
                    ({plumber.totalRating})
                  </span>
                </div>

                <p className="text__para text-[14px] leading-5 md:text-[15px] lg:max-w-[390px] ">
                  Lorem ipsum dolor sit amet consectetur, adipisicing elit.
                  Velit soluta, obcaecati architecto aut quidem qui est.
                </p>
              </div>
            </div>

            <div className="mt-[50px] border-b border-solid border-[#0066ff34] ">
              <button
                onClick={() => setTab("about")}
                className={`${
                  tab === "about" && "border-b border-solid border-primaryColor"
                } py-2 px-5 mr-5 text-[16px] texst-headingColor font-semibold`}
              >
                About
              </button>
              <button
                onClick={() => setTab("feedback")}
                className={`${
                  tab === "feedback" &&
                  "border-b border-solid border-primaryColor"
                } py-2 px-5 mr-5 text-[16px] texst-headingColor font-semibold`}
              >
                Feedback
              </button>
            </div>

            {plumber && (
              <div className="mt-[50px]">
                {tab === "about" && (
                  <PlumberAbout plumber={plumber} key={plumber._id} />
                )}
                {tab === "feedback" && (
                  <Feedback plumber={plumber} fetchData={fetchData} />
                )}
              </div>
            )}
          </div>
          <div>
            <SidePanel />
          </div>
        </div>
      </div>
    </section>
  );
}
